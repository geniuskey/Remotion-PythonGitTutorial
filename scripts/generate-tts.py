...
# - ko-KR-SunHiNeural (Female, natural) - Recommended
...
async def generate_tts(segment: dict) -> str:
    """Generate TTS audio for a single segment."""
    output_path = os.path.join(OUTPUT_DIR, f"{segment['id']}.mp3")
...
async def main():
    ...
    print("[TTS] Generating TTS audio files...\n")
    print(f"Voice: {VOICE}")
    ...
    for i, segment in enumerate(narration_script):
        try:
            await generate_tts(segment)
            print(f"[OK] [{i+1}/{len(narration_script)}] Generated: {segment['id']}.mp3")
            success_count += 1
        except Exception as e:
            print(f"[FAIL] [{i+1}/{len(narration_script)}] Failed: {segment['id']} - {e}")
            fail_count += 1

    print(f"\n[COMPLETE] Generated {success_count} audio files.")
    if fail_count > 0:
        print(f"[WARNING] Failed: {fail_count} files")

    print("\nNext steps:")
    print("1. Check the audio files in public/audio/")
    print("2. Run 'npm run dev' to preview the video")
